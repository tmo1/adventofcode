...
distances = [(0, 0, 0) for i in range(len(sensors))]
while sum(1 for i in range(len(sensors)) if sensors[i] is not None) > 1:
	#print(len(sensors), 'sensors remaining')
	for i in range(len(sensors) - 1):
		if sensors[i] is None:
			continue
		for j in range(i + 1, len(sensors)):
			if sensors[j] is None:
				continue
			...
			distances[j] = diff(distances[i], diff((0, 0, 0), d))
			sensors[j] = None
			break
		else:
			continue
...
for i in range(len(distances) - 1):
	for j in range(i + 1, len(distances)):
		d = diff(distances[i], distances[j])
		manhattans.add(sum(abs(d[i]) for i in range(3)))
